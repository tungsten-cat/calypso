...
UNIVERSAL_ASM_OBJECT_FILES := $(patsubst $(SOURCES_DIRECTORY)/%.asm, $(BUILD_DIRECTORY)/%.o, $(ASM_SOURCE_FILES))
...
build-amd64: $(ASM_OBJECT_FILES)
	...
	$(LD) $(LD_FLAGS) -o $(RELEASE_DIRECTORY)/calypso-$(TARGET_ARCH).bin -T $(TARGET_DIRECTORY)/linker.ld $(ASM_OBJECT_FILES) && \
	cp $(RELEASE_DIRECTORY)/$(TARGET_ARCH)/calypso-$(TARGET_ARCH).bin $(TARGET_DIRECTORY)/iso/boot/calypso-$(TARGET_ARCH).bin && \
	grub-mkrescue -o $(RELEASE_DIRECTORY)/calypso-$(TARGET_ARCH).bin $(TARGET_DIRECTORY)/iso